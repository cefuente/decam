...
nameroot='cen' # string the name of the files will begin with
...
ras = (array(range(len_ra)) - (len_ra-1)/2.)*step_ra*sub_ra # remember these will be 2x2 chuncks
ras = reshape(array(list(ras) *len_dec), (len_ra,-1))

#print ras

decs = (array(range(len_dec))  - (len_dec-1)/2.)*step_dec*sub_dec # remember these will be 2x2 chuncks
decs = reshape(array(list(decs) *len_ra), (len_dec,-1))
decs = decs.T # flip so that we don't get just the diagonal
decs  += numpy.ones(numpy.shape(decs))*decC# Add zeropoint

#print decs

# There's a difference when len_ra = 1
...
q = 0
## swaped len_dec, len_ra... is it right? 20140429
for i in range(len_dec):
    for j in range(len_ra):
        ...
        util = 'python point_coord_NEO_DECam.py --raC %f --decC %f --initfield %d --quadrant %d --fname %s --direction %s --sub_ra %d --sub_dec %d --exptime %d --filter %s --npasses %d --dirout %s %s'%(ras[i][j], decs[i][j], initfield, q, fname, direction, sub_ra, sub_dec, exptime, filter, npasses, dirout, verbose)
